...
export default function Modal(props: { isOpen: boolean, setIsOpen: Function, mainSettings: any, userSettings: { password: string, user: string } }) {
  ...
  const newUnmappedCollums = props.mainSettings.unusedCollumsLegalShield.map((currentItem: any) => {
    return <p className={` mt-2 ml-5 mr-5 text-[#E54800] whitespace-nowrap overflow-x-auto text-xl text-[#FF5100] cursor-default`} >{currentItem}</p>
  })
  ...
  const failedMappingText = (props.mainSettings.failedMappings.length === 0 ? ` text-[#FF5100] border-b-2 border-[#706E6C] text-xl  cursor-default` : ` text-[#ff0f0f] border-b-2 border-[#706E6C] text-xl  cursor-default`)
            ...
            >
              { props.userSettings.password === '' ?
                ...
                <DialogTitle as="h1" className="text-4xl justify-self-center self-center" >
                  ...
                  <div>
                    <p className="mt-2 text-lg ml-5 text-[#E54800]">Unmaped Collums: <span className='border-b-2 border-[#706E6C] text-xl text-[#FF5100] cursor-default' data-tooltip={`Legal Shield collums not mapped with a Field Notes Headers`}>{props.mainSettings.unusedCollumsLegalShield.length === 0 ? '0' : props.mainSettings.unusedCollumsLegalShield.length}</span>{ props.mainSettings.unusedCollumsLegalShield.length === 0 ? '' : <span><Button className={`float-right text-lg mr-5 px-3 border rounded-sm border-[#706E6C] bg-[#1B1917] hover:bg-[#292524]`} onClick={() => { handleToggleBox('unmappedCollums') }}>x</Button></span> }</p>
                    {toggleBox.unmappedCollums ? 
                      <div className='mt-1 p-1 pb-2 border rounded-sm border-[#706E6C] bg-[#1B1917] flex flex-col'>
                      <p className={`w-fit mt-2 ml-5 mr-5 text-[#E54800] border-b-2 border-[#706E6C] text-lg text-[#FF5100] cursor-default`}>Legal Shield Headers</p>
                      {newUnmappedCollums}
                    </div>
                      : ''
                    }
